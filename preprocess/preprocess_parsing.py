...
def find_pair(data):
    def check_author(path, op):
        return sum([ 1 if(_['author']==data['author']) else 0 for _ in path])
    ...
    temp = []
    for path in count[0]:
        c_author = [op_author, path[0]['author']]
        flag = False
        
        for _ in path:
            if(_['author'] not in c_author):
                break
            elif(_['author'] == op_author):
                flag = True
        else:
            if(flag):
                temp.append(path)
